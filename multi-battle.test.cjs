...
vm.runInContext(`
  ...
  towerRun={floor:31,team:[{sp:SP_BY_ID.duraludon,item:'none'},{sp:SP_BY_ID.gyarados,item:'none'},{sp:SP_BY_ID.clobbopus,item:'none'}]};saveTowerProgress();const towerSave=JSON.parse(localStorage.getItem(TOWER_SAVE_KEY));check(typeof chooseTowerTeam==='function'&&typeof resumePokemonTower==='function'&&typeof saveAndExitTower==='function'&&towerSave.floor===31&&towerSave.team.length===3,'Tower saves floor and selected team and provides reselection/resume/exit actions');
...
vm.runInContext(`
  ...
  sayaku.curHp=Math.floor(sayaku.maxHp/2);sayakuTarget.justSwitched=true;sayakuTarget.switchLock=0;sayakuTarget.curHp=sayakuTarget.maxHp;const dullFangDamage=calcDamage(sayaku,sayakuTarget,{...M.sayakuDullFang,power:M.sayakuDullFang.power*2},false).dmg;attack(sayaku,sayakuTarget,M.sayakuDullFang,'foeImg',()=>{});check(sayakuTarget.switchLock===4&&sayakuTarget.curHp===sayakuTarget.maxHp-dullFangDamage,'Sayaku Torrent keeps Dull Fang switched-target damage at two times while doubling its switch lock');
