...
vm.runInContext(`
  ...
  check(['kokugaiSludgeWave','sludgebomb','hex','toxicspikes','meteor','nastyPlot','grassknot','hydropump','crunch','jawlock','acidspray','focusblast','flipturn','flamethrower','kokugaiDragonBreath'].every(k=>SPECIES[SP_BY_ID.kokugainoryu].learnset.includes(k)&&M[k]),'Kokugainoryu learns all requested additional moves');
  ...
  const gammaTarget=makeMon(SP_BY_ID.gyarados,'none','cpu');gammaTarget.maxHp=gammaTarget.curHp=100000;me=kokugainoryu;foe=gammaTarget;myTeam=[kokugainoryu];foeTeam=[gammaTarget];kokugainoryu.magicGauge=0;kokugainoryu.gammaRadiationTargets=new Set();const oldGammaRandom=Math.random;Math.random=()=>.4;attack(kokugainoryu,gammaTarget,M.gammaRadiation,'foeImg',()=>{});Math.random=oldGammaRandom;const gammaTargetHp=gammaTarget.curHp;check(M.gammaRadiation.power===10&&M.gammaRadiation.multiHit===10&&kokugainoryu.magicGauge===20&&gammaTarget.radiationGauge===15&&stgText(gammaTarget).includes('☢ 放射能 15%'),'Gamma Radiation hits ten times for 20 percent Magic and visibly adds radiation');attack(kokugainoryu,gammaTarget,M.gammaRadiation,'foeImg',()=>{});check(gammaTarget.curHp===gammaTargetHp,'Gamma Radiation cannot be used on the same target twice');
`,c);
