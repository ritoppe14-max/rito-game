...
vm.runInContext(`
  ...
  const normalBenchDamage=calcDamage(decidueye,benchTarget,M.shadowStitch,false).dmg;attack(decidueye,benchTarget,{...M.shadowStitch,longReachBench:true},'foeImg',()=>{});Math.random=savedLongReachRandom;check(benchTarget.curHp===benchHp-Math.floor(normalBenchDamage*.3),'Long Reach damages only the selected bench target at 30 percent power');
