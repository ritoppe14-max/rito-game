...
vm.runInContext(`
  ...
  const fixedTowerFoes=buildTowerFoes(31);towerRun={floor:31,team:[{sp:SP_BY_ID.duraludon,item:'none'},{sp:SP_BY_ID.gyarados,item:'none'},{sp:SP_BY_ID.clobbopus,item:'none'}],foes:fixedTowerFoes,foesFloor:31};saveTowerProgress();const towerSave=JSON.parse(localStorage.getItem(TOWER_SAVE_KEY));check(typeof chooseTowerTeam==='function'&&typeof resumePokemonTower==='function'&&typeof offerTowerSave==='function'&&typeof confirmTowerSave==='function'&&towerSave.floor===31&&towerSave.team.length===3&&towerSave.foesFloor===31&&Array.isArray(towerSave.foes)&&towerSave.foes.length===fixedTowerFoes.length&&!startTowerFloor.toString().includes('saveTowerProgress()'),'Tower saves only from its post-win save window, preserves the fixed foe roster, and can resume after a loss');
