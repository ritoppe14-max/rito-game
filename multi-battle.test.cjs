...
vm.runInContext(`
  ...
  const raticate=makeMon(SP_BY_ID.raticate,'none','p1'),butterfree=makeMon(SP_BY_ID.butterfree,'none','p1'),beedrill=makeMon(SP_BY_ID.beedrill,'none','p1'),pidgeot=makeMon(SP_BY_ID.pidgeot,'none','p1');check(raticate.img==='image/image/ラッタ.gif'&&raticate.types.join('/')==='normal'&&raticate.base.atk===81&&butterfree.img==='image/image/バタフリー.gif'&&butterfree.types.join('/')==='bug/flying'&&butterfree.abilEff==='compoundeyes'&&beedrill.img==='image/image/スピアー.gif'&&beedrill.types.join('/')==='bug/poison'&&beedrill.abilEff==='sniper'&&pidgeot.img==='image/image/ピジョット.gif'&&pidgeot.types.join('/')==='normal/flying'&&pidgeot.base.spe===101,'Raticate, Butterfree, Beedrill and Pidgeot have requested images, typings and core data');
  const arbok=makeMon(SP_BY_ID.arbok,'none','p1'),fearow=makeMon(SP_BY_ID.fearow,'none','p1'),sandslash=makeMon(SP_BY_ID.sandslash,'none','p1'),nidoqueen=makeMon(SP_BY_ID.nidoqueen,'none','p1'),nidoking=makeMon(SP_BY_ID.nidoking,'none','p1');check(arbok.img==='image/image/アーボック.gif'&&arbok.types.join('/')==='poison'&&arbok.abilEff==='intimidate'&&fearow.img==='image/image/オニドリル.gif'&&fearow.types.join('/')==='normal/flying'&&fearow.base.spe===100&&sandslash.img==='image/image/サンドパン.gif'&&sandslash.types.join('/')==='ground'&&sandslash.base.def===140&&nidoqueen.img==='image/image/ニドクイン.gif'&&nidoqueen.types.join('/')==='poison/ground'&&nidoqueen.abilEff==='sheerforce'&&nidoking.img==='image/image/ニドキング.gif'&&nidoking.types.join('/')==='poison/ground'&&nidoking.abilEff==='sheerforce','Arbok, Fearow, Sandslash, Nidoqueen and Nidoking have requested images, typings and core data');
  const alakazam=makeMon(SP_BY_ID.alakazam,'none','p1'),victreebel=makeMon(SP_BY_ID.victreebel,'victreebelite','p1');check(alakazam.img==='image/image/フーディン.gif'&&alakazam.types.join('/')==='psychic'&&alakazam.base.spa===135&&alakazam.abilEff==='magicguard'&&victreebel.img==='image/image/ウツボット.gif'&&victreebel.types.join('/')==='grass/poison'&&victreebel.abilEff==='chlorophyll','Alakazam and Victreebel have requested images, typings and core data');doMega(victreebel,'myImg',()=>{});check(victreebel.isMega&&victreebel.name==='メガウツボット'&&victreebel.img==='image/image/メガウツボット.gif'&&victreebel.abilEff==='chlorophyll','Victreebelite Mega Evolves Victreebel into Mega Victreebel');
  const jynx=makeMon(SP_BY_ID.jynx,'none','p1'),tauros=makeMon(SP_BY_ID.tauros,'none','p1');check(jynx.img==='image/image/ルージュラ.gif'&&jynx.types.join('/')==='ice/psychic'&&jynx.base.spa===115&&tauros.img==='image/image/ケンタロス.gif'&&tauros.types.join('/')==='normal'&&tauros.base.atk===100&&tauros.abilEff==='taurosforce'&&M.hyperbeam.recharge,'Jynx and Tauros have requested images, typings and Tauros special effect data');
  const snorlax=makeMon(SP_BY_ID.snorlax,'none','p1');check(snorlax.img==='image/image/カビゴン.gif'&&snorlax.types.join('/')==='normal'&&snorlax.base.hp===160&&snorlax.base.atk===110&&snorlax.base.def===95&&snorlax.base.spd===140&&snorlax.abilEff==='thickfat'&&M.sleeptalk.sleeptalk&&M.rest.rest,'Snorlax has requested stats, image and Sleep Talk full-heal setup');
  const breloom=makeMon(SP_BY_ID.breloom,'none','p1');check(breloom.img==='image/image/キノガッサ.gif'&&breloom.types.join('/')==='grass/fighting'&&breloom.base.hp===60&&breloom.base.atk===130&&breloom.base.def===110&&breloom.base.spa===60&&breloom.base.spd===90&&breloom.base.spe===70&&breloom.abilEff==='technician'&&['spore','machpunch','bulletseed','seedbomb'].every(k=>SPECIES[SP_BY_ID.breloom].learnset.includes(k)&&M[k]),'Breloom has requested image, Grass/Fighting stats, Technician and moves');
  const gothitelle=makeMon(SP_BY_ID.gothitelle,'none','p1'),seismitoad=makeMon(SP_BY_ID.seismitoad,'none','p1'),zoroark=makeMon(SP_BY_ID.zoroark,'none','p1'),zebstrika=makeMon(SP_BY_ID.zebstrika,'none','p1');check(gothitelle.img==='image/image/ゴチルゼル.gif'&&gothitelle.types.join('/')==='psychic'&&gothitelle.base.spa===95&&seismitoad.img==='image/image/ガマゲロゲ.gif'&&seismitoad.types.join('/')==='water/ground'&&seismitoad.abilEff==='waterabsorb'&&zoroark.img==='image/image/ゾロアーク.gif'&&zoroark.types.join('/')==='dark'&&zoroark.abilEff==='illusion'&&zebstrika.img==='image/image/ゼブライカ.gif'&&zebstrika.types.join('/')==='electric'&&zebstrika.base.spe===116,'Gothitelle, Seismitoad, Zoroark and Zebstrika have requested images, typings and core data');
